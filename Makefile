...
generate:
	...
	go run $(OAPI) --package $(PACKAGE)api --generate types --include-tags $(TAGS) -o $(BASE)/$(PACKAGE)/types.gen.go $(SPEC)
	go run $(OAPI) --package $(PACKAGE)api --generate client --include-tags $(TAGS) -o $(BASE)/$(PACKAGE)/client.gen.go $(SPEC)
